...
import teamunc.uncsurvival.eventsListeners.events.ArmorEquipEvent;
...
import teamunc.uncsurvival.utils.ArmorType;
...
public class ArmorListener extends AbstractEventsListener implements Listener  {

    private final List<String> blockedMaterials;

    public ArmorListener(List<String> blockedMaterials, UNCSurvival plugin){
        super(plugin);
        this.blockedMaterials = blockedMaterials;
    }

    @EventHandler
    public final void ArmorEquipEvent(ArmorEquipEvent event) {
        Player p = event.getPlayer();
        PlayerInventory inv = p.getInventory();
        List<String> armors = List.of("AMETHYSTHELMET", "AMETHYSTCHESTPLATE", "AMETHYSTLEGGINGS", "AMETHYSTBOOTS");
        List<EquipmentSlot> slots = new ArrayList<>();
        slots.addAll(List.of(EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET));
        slots.remove(this.convertEquipmentSlot(event.getType()));
        for(EquipmentSlot slot : slots) {
            if(inv.getItem(slot) == null || !this.plugin.getGameManager().getItemsManager().isCustomItemOfArray(inv.getItem(slot), armors)) {
                return;
            }
        }
        if(!this.plugin.getGameManager().getItemsManager().isCustomItemOfArray(event.getNewArmorPiece(), armors)) {
            return;
        }
        // advancement
    ...
    public EquipmentSlot convertEquipmentSlot(ArmorType armor) {
        EquipmentSlot index;
        switch(armor) {
            case BOOTS:
                index = EquipmentSlot.FEET;
                break;
            case HELMET:
                index = EquipmentSlot.HEAD;
                break;
            case CHESTPLATE:
                index = EquipmentSlot.CHEST;
                break;
            case LEGGINGS:
                index = EquipmentSlot.LEGS;
                break;
            default:
                index = EquipmentSlot.OFF_HAND;
        }
        return index;
    }
    ...
    public final void inventoryClick(final InventoryClickEvent e){
        ...
        if(e.getSlotType() != InventoryType.SlotType.ARMOR && e.getSlotType() != SlotType.QUICKBAR && e.getSlotType() != SlotType.CONTAINER) return;
