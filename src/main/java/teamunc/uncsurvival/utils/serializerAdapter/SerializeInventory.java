...
public class SerializeInventory implements JsonSerializer<Inventory>, JsonDeserializer<Inventory> {
    /**
    ...
    public Inventory deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
        JsonObject jsonInv = jsonElement.getAsJsonObject();
        InventoryType inventoryType = InventoryType.valueOf(jsonInv.get("type").getAsString());
        ItemStack[] itemStacks = null;
        try {
            itemStacks = itemStackArrayFromBase64(jsonInv.get("content").getAsString());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public JsonElement serialize(Inventory itemStacks, Type type, JsonSerializationContext jsonSerializationContext) {
        ...
        jsonObject.addProperty("content", itemStackArrayToBase64(itemStacks.getContents()));
        jsonObject.addProperty("type", itemStacks.getType().toString());
        return jsonObject;
    }
}